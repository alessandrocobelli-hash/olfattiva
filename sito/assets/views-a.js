...
(function () {
  ...
  /* ---------- componenti */
  var C = OLF.c = {};
  C.lon = function (n) { var s = '<span class="lon" role="img" aria-label="Persistenza ' + (n || 3) + ' su 5">', i; for (i = 1; i <= 5; i++) s += '<i class="' + (i <= (n || 3) ? 'on' : '') + '"></i>'; return s + '</span>'; };
  ...
  C.pyramid = function (p) {
    ...
    return '<div class="pyr">' + rows.map(function (r) {
      return '<div class="pyr-row"><div class="pyr-lab stack-sm"><span class="eyebrow" style="color:var(--night-ink)">' + r[1] + '</span><span class="cap">' + r[2] + '</span></div><div class="pyr-box"><div class="pyr-in" style="width:' + r[3] + '%">' +
  ...
  V.shell = function (route, inner) {
    ...
    return '<div class="shell"><nav class="nav" aria-label="Principale"><a class="logo" href="#/oggi">Olfattiva</a><div class="nav-links">' + links + '</div>' +
  ...
  V.guardaroba = function (sub) {
    ...
    var m = E.matrix();
    out += '<section class="band night"><div class="row between end" style="gap:32px;margin-bottom:24px"><div class="stack-sm"><div class="eyebrow">Copertura del guardaroba</div><h2 class="h2">Quando sei vestito, e quando no</h2></div><p class="muted" style="max-width:460px;font-size:15px">' +
      (m.tagged < m.total ? 'Hai etichettato ' + m.tagged + ' flaconi su ' + m.total + ': apri ogni profumo presente e indica le occasioni per riempire la matrice.' : 'Le caselle ambra sono i momenti in cui oggi non hai niente di tuo da mettere.') + '</p></div><div class="matrix"><div></div>' + E.SEAS.map(function (s) { return '<div class="hd">' + s[1] + '</div>'; }).join('') +
  ...
  V.wardrobeAdd = function () {
    ...
    out += '<div class="cols"><div class="grow stack"><div class="field"><label for="addq">Cerca nel catalogo</label><input class="input" id="addq" type="search" value="' + h(UI.addQuery || '') + '" placeholder="es. Interlude, Tom Ford, Kilian" data-input="addQuery"></div><div class="list" id="addres">' +
      res.map(function (p) { return '<div><span class="stack-sm"><span class="t">' + h(p.name) + '</span><span class="s">' + C.meta(p) + '</span></span><span class="row" style="gap:6px"><button class="btn sm" data-act="addItem" data-pid="' + p.id + '" data-status="present">Ce l’ho</button><button class="btn sm" data-act="addItem" data-pid="' + p.id + '" data-status="past">L’ho avuto</button><button class="btn sm" data-act="addItem" data-pid="' + p.id + '" data-status="future">Lo voglio</button></span></div>'; }).join('') +
  ...
  V.wardrobeEdit = function (pid) {
    ...
    return C.head('Guardaroba', h(p.name), C.meta(p) + ' · ' + h(p.fam), '<a class="btn" href="#/profumo/' + p.id + '">Apri la scheda</a>') +
      ...
      '<aside class="side stack"><div class="card"><span class="eyebrow">Diario degli indossi</span><span class="big">' + (w.wears || []).length + '</span><span class="cap">' + (E.lastWorn(w) ? 'Ultima volta: ' + E.lastWorn(w) : 'Mai segnato') + '</span><button class="btn sm" type="button" data-act="wearToday" data-pid="' + p.id + '">Indossato oggi</button></div>' +
