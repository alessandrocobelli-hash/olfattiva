...
(function () {
  ...
  V.guardaroba = function (sub) {
    ...
    out += '<section class="band night"><div class="row between end" style="gap:32px;margin-bottom:24px"><div class="stack-sm"><div class="eyebrow">Copertura del guardaroba</div><h2 class="h2">Quando sei vestito, e quando no</h2></div><p class="muted" style="max-width:460px;font-size:15px">' +
      (m.tagged < m.total ? 'Questa matrice nasce da quello che dichiari su ogni flacone che possiedi, non da quando lo indossi davvero. Apri un profumo presente dal guardaroba qui sopra, scorri fino a "Stagioni" e "Occasioni" e seleziona quelle giuste: ogni casella qui sotto si riempie da sola. Per ora hai etichettato ' + m.tagged + ' flaconi su ' + m.total + '.' : 'Tutti i flaconi presenti sono etichettati. Le caselle ambra sono i momenti in cui oggi non hai niente di tuo da mettere: cliccale per vedere proposte.') + '</p></div>' +
      (m.tagged < m.total ? '<a class="btn sm" href="#/guardaroba">Vai a etichettare</a>' : '') + '</div><div class="matrix"><div></div>' + E.SEAS.map(function (s) { return '<div class="hd">' + s[1] + '</div>'; }).join('') +
