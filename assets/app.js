...
(function () {
  ...
  A.catMore = function () { UI.cat.n = (UI.cat.n || 150) + 150; render(); };
  ...
  function onInput(el) {
    ...
    if (k === 'catQuery' || k === 'catNote' || k === 'catYFrom' || k === 'catYTo') {
      var c = UI.cat = UI.cat || { q: '', fam: '', lon: false, sort: 'fit', n: 150, brand: '', note: '', mol: '', yFrom: '', yTo: '' };
      var map = { catQuery: 'q', catNote: 'note', catYFrom: 'yFrom', catYTo: 'yTo' };
      c[map[k]] = el.value; c.n = 150;
    } else UI[k] = el.value;
    render(INPUTS[k]);
  }
  CH.catFam = function (el) { UI.cat.fam = el.value; UI.cat.n = 150; render(); };
  CH.catBrand = function (el) { UI.cat.brand = el.value; UI.cat.n = 150; render(); };
  CH.catMol = function (el) { UI.cat.mol = el.value; UI.cat.n = 150; render(); };
  CH.catSort = function (el) { UI.cat.sort = el.value; UI.cat.n = 150; render(); };
  CH.catLon = function (el) { UI.cat.lon = el.checked; UI.cat.n = 150; render(); };
  A.catReset = function () { UI.cat = { q: '', fam: '', lon: false, sort: 'fit', n: 150, brand: '', note: '', mol: '', yFrom: '', yTo: '' }; render(); };
