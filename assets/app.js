...
(function () {
  ...
  function render(keepId) {
    ...
    if (r.route === 'affinita' && V._affinitaFar) requestAnimationFrame(V._affinitaFar);
  ...
  A.addWear = function (el) {
    var w = E.item(el.dataset.pid); if (!w) return;
    var inp = document.getElementById('weardt'), t = (inp && inp.value) || OLF.today();
    if (w.wears.indexOf(t) < 0) { w.wears.push(t); if (w.status !== 'present') w.status = 'present'; save(); OLF.toast('Aggiunto al diario'); }
    render();
  };
  A.removeWear = function (el) {
    var w = E.item(el.dataset.pid); if (!w) return;
    var i = w.wears.indexOf(el.dataset.date); if (i >= 0) { w.wears.splice(i, 1); save(); }
    render();
  };
  ...
  A.catPage = function (el) { UI.cat.page = +el.dataset.page; render(); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  A.catLetter = function (el) { UI.cat.letter = el.dataset.letter; UI.cat.page = 1; render(); };
  A.toggleMenu = function () { UI.menuOpen = !UI.menuOpen; render(); };

  /* ---------- zoom e trascinamento sulla mappa di affinità */
  function mapView() { return UI.mapView = UI.mapView || { k: 1, x: 0, y: 0 }; }
  function applyMapTransform() { var el = document.getElementById('mapInner'), v = mapView(); if (el) el.style.transform = 'translate(' + v.x + 'px,' + v.y + 'px) scale(' + v.k + ')'; }
  A.zoomIn = function () { var v = mapView(); v.k = Math.min(4, +(v.k + 0.4).toFixed(2)); applyMapTransform(); };
  A.zoomOut = function () { var v = mapView(); v.k = Math.max(1, +(v.k - 0.4).toFixed(2)); if (v.k === 1) { v.x = 0; v.y = 0; } applyMapTransform(); };
  A.zoomReset = function () { UI.mapView = { k: 1, x: 0, y: 0 }; applyMapTransform(); };
  A.toTop = function () { window.scrollTo({ top: 0, behavior: 'smooth' }); };
  window.addEventListener('scroll', function () { var b = document.getElementById('toTop'); if (b) b.classList.toggle('show', window.scrollY > 480); }, { passive: true });
  (function () {
    var dragging = false, moved = false, sx = 0, sy = 0, ox = 0, oy = 0;
    document.addEventListener('pointerdown', function (ev) {
      var wrap = ev.target.closest && ev.target.closest('.map-wrap'); if (!wrap) return;
      var v = mapView(); dragging = true; moved = false; sx = ev.clientX; sy = ev.clientY; ox = v.x; oy = v.y;
      wrap.classList.add('dragging'); wrap.setPointerCapture && ev.pointerId != null && wrap.setPointerCapture(ev.pointerId);
    });
    document.addEventListener('pointermove', function (ev) {
      if (!dragging) return;
      var dx = ev.clientX - sx, dy = ev.clientY - sy;
      if (Math.abs(dx) > 3 || Math.abs(dy) > 3) moved = true;
      var v = mapView(); v.x = ox + dx; v.y = oy + dy; applyMapTransform();
    });
    function stop(ev) {
      if (!dragging) return; dragging = false;
      var wrap = document.getElementById('mapWrap'); if (wrap) wrap.classList.remove('dragging');
      if (moved && ev && ev.preventDefault) ev.preventDefault(); // evita che il trascinamento apra una scheda
    }
    document.addEventListener('pointerup', stop);
    document.addEventListener('pointercancel', stop);
    document.addEventListener('click', function (ev) { if (moved && ev.target.closest && ev.target.closest('.map-wrap')) { ev.stopPropagation(); moved = false; } }, true);
  })();

  /* ---------- campi che aggiornano la vista */
  var INPUTS = { addQuery: 'addq', noteQuery: 'nq', catQuery: 'cq', catNote: 'cnote', catYFrom: 'cyfrom', catYTo: 'cyto', shopBrand: 'sb', maisonQuery: 'mq' };
  function onInput(el) {
    var k = el.dataset.input;
    if (k === 'catQuery' || k === 'catNote' || k === 'catYFrom' || k === 'catYTo') {
      var c = UI.cat = UI.cat || { q: '', fam: '', lon: false, sort: 'fit', page: 1, brand: '', note: '', mol: '', yFrom: '', yTo: '', letter: '' };
      var map = { catQuery: 'q', catNote: 'note', catYFrom: 'yFrom', catYTo: 'yTo' };
      c[map[k]] = el.value; c.page = 1;
    } else UI[k] = el.value;
    render(INPUTS[k]);
  }
  CH.catFam = function (el) { UI.cat.fam = el.value; UI.cat.page = 1; render(); };
  CH.catBrand = function (el) { UI.cat.brand = el.value; UI.cat.page = 1; render(); };
  CH.catMol = function (el) { UI.cat.mol = el.value; UI.cat.page = 1; render(); };
  CH.catSort = function (el) { UI.cat.sort = el.value; UI.cat.page = 1; render(); };
  CH.catLon = function (el) { UI.cat.lon = el.checked; UI.cat.page = 1; render(); };
  A.catReset = function () { UI.cat = { q: '', fam: '', lon: false, sort: 'fit', page: 1, brand: '', note: '', mol: '', yFrom: '', yTo: '', letter: '' }; render(); };
  ...
  window.addEventListener('hashchange', function () { UI.menuOpen = false; if (OLF.DATA) { render(); window.scrollTo(0, 0); } });
  ...
  (async function boot() {
    if ('serviceWorker' in navigator && (location.protocol === 'https:' || location.hostname === 'localhost')) {
      ...
      // quando un service worker nuovo prende il controllo (nuovo deploy), ricarica una sola volta:
      // altrimenti l'utente continua a vedere la versione vecchia finché non chiude e riapre il sito da solo
      var reloaded = false;
