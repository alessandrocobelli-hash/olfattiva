...
(function () {
  ...
  function declutter(pts, ids) {
    for (var it = 0; it < 60; it++) for (var i = 0; i < ids.length; i++) for (var j = i + 1; j < ids.length; j++) {
  ...
  // Layout in due passate: la costosa distensione anti-sovrapposizione (60 iterazioni × N²) gira
  // solo sui punti "vicini" mostrati subito (di norma ≤200); il resto usa la posizione grezza,
  // molto più economica su un catalogo di migliaia di profumi.
  function layout(Wd, Hd, nearIds) {
    var pts = {}, allIds = [];
    E.all().forEach(function (p) { pts[p.id] = rawXY(p, Wd, Hd); allIds.push(p.id); });
    declutter(pts, nearIds && nearIds.length ? nearIds : allIds.slice(0, 200));
  ...
  V.affinita = function (seedId) {
    ...
    E.neighbours(seed, 'none', 190).forEach(function (n) { if (!nearSet[n.p.id]) { nearSet[n.p.id] = 1; nearIds.push(n.p.id); } });
  ...
  /* ---------- Catalogo e scheda */
  V.catalogo = function () {
    var f = UI.cat = UI.cat || { q: '', fam: '', lon: false, sort: 'fit', n: 150, brand: '', note: '', mol: '', yFrom: '', yTo: '' }, q = f.q.toLowerCase();
    ...
    var shown = list.slice(0, f.n || 150);
    var activeFilters = [f.fam, f.brand, f.note, f.mol, f.yFrom, f.yTo].some(Boolean);
    return C.head('Catalogo · ' + E.all().length + ' schede', 'Tutti i <em>profumi</em>', 'Piramidi da fonti pubbliche, persistenza come stima editoriale. Buona parte del catalogo è importata in blocco da un database pubblico: famiglia e persistenza automatiche, correggibili scheda per scheda.', '<a class="btn" href="#/guardaroba/nuovo">Aggiungi una scheda</a>') +
      ...
      '<p class="cap muted" style="margin:0 0 4px">La percentuale a destra è l’<strong>affinità</strong> con il tuo profilo attuale (note e famiglie che ami di più), non la persistenza — quella è il pallino a sinistra.' + (f.mol ? ' Solo i profumi con attribuzione verificata a mano hanno una molecola collegata: gli altri non compaiono qui anche se probabilmente la contengono.' : '') + '</p>' +
      '<p class="cap muted" style="margin:0 0 14px">Mostro ' + shown.length + ' su ' + list.length + ' risultati.</p>' +
      '<div class="list" id="catlist">' + V.catRows(shown) + '</div>' +
      (list.length > shown.length ? '<div class="row" style="justify-content:center;padding:20px 0"><button class="btn" data-act="catMore">Carica altri 150</button></div>' : '');
  };
