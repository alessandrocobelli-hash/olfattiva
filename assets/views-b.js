...
(function () {
  ...
  /* ---------- mappa dell'affinità */
  var RAWXY_CACHE = null; // le posizioni grezze dipendono solo dal catalogo: si calcolano una volta sola per sessione
  function rawXY(p, Wd, Hd) {
    var a = E.axes(p), wx = Math.min(Math.max(a.warm, -0.7), 0.9), sy = Math.min(Math.max(a.sweet, -0.85), 0.65);
    return { x: 30 + (wx + 0.7) / 1.6 * (Wd - 60), y: 34 + (sy + 0.85) / 1.5 * (Hd - 68) };
  }
  function declutter(pts, ids) {
    for (var it = 0; it < 24; it++) for (var i = 0; i < ids.length; i++) for (var j = i + 1; j < ids.length; j++) {
  ...
  // Layout in due passate: la costosa distensione anti-sovrapposizione gira solo sui punti "vicini"
  // mostrati subito (≤130); il resto usa la posizione grezza. Le posizioni grezze, uguali per tutta
  // la sessione, si calcolano una volta sola e si clonano ad ogni apertura della mappa.
  function layout(Wd, Hd, nearIds) {
    var key = Wd + 'x' + Hd, pts = {}, allIds = [];
    if (RAWXY_CACHE && RAWXY_CACHE.key === key) {
      allIds = RAWXY_CACHE.ids;
      allIds.forEach(function (id) { var r = RAWXY_CACHE.pts[id]; pts[id] = { x: r.x, y: r.y }; });
    } else {
      E.all().forEach(function (p) { pts[p.id] = rawXY(p, Wd, Hd); allIds.push(p.id); });
      var frozen = {}; allIds.forEach(function (id) { frozen[id] = { x: pts[id].x, y: pts[id].y }; });
      RAWXY_CACHE = { key: key, ids: allIds, pts: frozen };
    }
    declutter(pts, nearIds && nearIds.length ? nearIds : allIds.slice(0, 130));
    allIds.forEach(function (id) { pts[id].x = Math.min(Math.max(pts[id].x, 12), Wd - 12); pts[id].y = Math.min(Math.max(pts[id].y, 16), Hd - 12); });
  ...
  V.affinita = function (seedId) {
    ...
    UI.seed = seed.id;
    var nb = E.neighbours(seed, UI.dir, 6), near = {}; nb.forEach(function (n) { near[n.p.id] = 1; });
    var st = {}; E.items().forEach(function (x) { st[x.p.id] = x.w.status; });
    // set "vicino": seme, guardaroba, e i ~190 profumi più affini al seme. Solo su questi gira la
    // distensione anti-sovrapposizione (costosa); il resto della mappa (migliaia di punti) resta a
    // posizione grezza e si aggiunge subito dopo, senza bloccare il primo disegno.
    var nearIds = [seed.id], nearSet = {}; nearSet[seed.id] = 1;
    Object.keys(st).forEach(function (id) { if (!nearSet[id]) { nearSet[id] = 1; nearIds.push(id); } });
    E.neighbours(seed, 'none', 130).forEach(function (n) { if (!nearSet[n.p.id]) { nearSet[n.p.id] = 1; nearIds.push(n.p.id); } });
    var pts = layout(Wd, Hd, nearIds);
    ...
    svg.push('<g id="mapFar"></g>'); // i punti lontani vengono iniettati qui poco dopo il primo disegno
    var er = E.eras();
    if (er.ready) {
    ...
    E.all().forEach(function (p) {
      ...
      if (!nearSet[p.id]) { farList.push({ id: p.id, x: pt.x, y: pt.y, s: s || '', low: low }); return; }
    ...
    // i punti lontani (di norma migliaia) vengono disegnati un istante dopo, senza etichette né
    // focus da tastiera: servono da contesto visivo, restano comunque cliccabili.
    V._affinitaFar = function () {
      var slot = document.getElementById('mapFar'); if (!slot) return;
      var out = farList.map(function (f) {
        var style = f.s === 'present' ? 'fill:#6E7972' : (f.s === 'past' ? 'fill:#4B544E' : (f.s === 'future' ? 'fill:none;stroke:#8C6A4A' : 'fill:none;stroke:#3B453F' + (f.low ? ';stroke-dasharray:1.5 2' : '')));
        return '<circle cx="' + f.x.toFixed(1) + '" cy="' + f.y.toFixed(1) + '" r="3.4" style="' + style + '" data-act="seed" data-pid="' + f.id + '"/>';
      }).join('');
      slot.innerHTML = out;
    };
    var placed = [];
    ...
    var mv = UI.mapView = UI.mapView || { k: 1, x: 0, y: 0 };
    out += '<div class="cols"><div class="grow stack" style="gap:12px"><div class="map-wrap" id="mapWrap"><div class="map-inner" id="mapInner" style="transform:translate(' + mv.x + 'px,' + mv.y + 'px) scale(' + mv.k + ')">' + svg.join('') + '</div><div class="map-zoom"><button class="btn sm ghost" type="button" data-act="zoomOut" aria-label="Riduci lo zoom">−</button><button class="btn sm ghost" type="button" data-act="zoomReset" aria-label="Reimposta la vista">•</button><button class="btn sm ghost" type="button" data-act="zoomIn" aria-label="Aumenta lo zoom">+</button></div><p class="cap mono map-hint">Trascina per spostarti, usa +/− per lo zoom</p></div><div class="row cap" style="gap:16px"><span class="row" style="gap:6px"><span style="width:11px;height:11px;border-radius:9px;background:var(--ink)"></span>presente</span><span class="row" style="gap:6px"><span style="width:11px;height:11px;border-radius:9px;background:#8C968F"></span>passato</span>' +
  ...
  V.note = function (name) {
    ...
    if (!name) {
      ...
      return C.head('Note e materie prime', 'Da una <em>nota</em> a tutto il resto', 'Ogni nota apre tre strade: la materia prima, i profumi che la contengono, le marche che la usano.') +
        '<div class="row" style="gap:18px;margin:-10px 0 22px;flex-wrap:wrap"><span class="row cap" style="gap:8px"><span class="chip on" style="min-height:22px;padding:0 10px;pointer-events:none">Esempio</span>già nel tuo guardaroba (presente o passato)</span><span class="row cap" style="gap:8px"><span class="chip" style="min-height:22px;padding:0 10px;pointer-events:none">Esempio</span>nel catalogo, non ancora provata da te</span></div>' +
  ...
  /* ---------- Catalogo e scheda */
  V.catalogo = function () {
    var f = UI.cat = UI.cat || { q: '', fam: '', lon: false, sort: 'fit', page: 1, brand: '', note: '', mol: '', yFrom: '', yTo: '', letter: '' }, q = f.q.toLowerCase();
    var brands = Array.from(new Set(E.all().map(function (p) { return p.brand; }))).sort(function (a, b) { return a.localeCompare(b); });
    var mols = OLF.DATA.molecules || [];
    var molSet = f.mol ? {} : null; if (f.mol) { var mo = mols.filter(function (m) { return m.id === f.mol; })[0]; (mo ? mo.perfumes : []).forEach(function (id) { molSet[id] = 1; }); }
    var noteQ = (f.note || '').toLowerCase();
    var list = E.all().filter(function (p) {
      if (q && (p.name + ' ' + p.brand + ' ' + (p.nose || '')).toLowerCase().indexOf(q) < 0) return false;
      if (f.fam && p.fam !== f.fam) return false;
      if (f.lon && !E.lonOk(p)) return false;
      if (f.brand && p.brand !== f.brand) return false;
      if (f.letter && p.name[0].toUpperCase() !== f.letter) return false;
      if (noteQ && p.top.concat(p.heart, p.base).join(' | ').toLowerCase().indexOf(noteQ) < 0) return false;
      if (f.mol && !molSet[p.id]) return false;
      if (f.yFrom && (!p.year || p.year < +f.yFrom)) return false;
      if (f.yTo && (!p.year || p.year > +f.yTo)) return false;
      return true;
    });
    var sorters = { fit: function (a, b) { return E.fit(b) - E.fit(a); }, year: function (a, b) { return (b.year || 0) - (a.year || 0); }, name: function (a, b) { return a.name.localeCompare(b.name); }, brand: function (a, b) { return a.brand.localeCompare(b.brand) || a.name.localeCompare(b.name); }, lon: function (a, b) { return (b.lon || 3) - (a.lon || 3); } };
    list.sort(sorters[f.sort] || sorters.fit);
    var PAGESZ = 100, pages = Math.max(1, Math.ceil(list.length / PAGESZ)), page = Math.min(Math.max(1, f.page || 1), pages);
    var shown = list.slice((page - 1) * PAGESZ, page * PAGESZ);
    var activeFilters = [f.fam, f.brand, f.note, f.mol, f.yFrom, f.yTo, f.letter].some(Boolean);
    var LETTERS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('');
    return C.head('Catalogo · ' + E.all().length + ' schede', 'Tutti i <em>profumi</em>', 'Piramidi da fonti pubbliche, persistenza come stima editoriale. Buona parte del catalogo è importata in blocco da un database pubblico: famiglia e persistenza automatiche, correggibili scheda per scheda.', '<a class="btn" href="#/guardaroba/nuovo">Aggiungi una scheda</a>') +
      '<div class="row end" style="margin-bottom:14px;gap:14px 18px"><div class="field grow" style="min-width:200px"><label for="cq">Cerca per nome, marca o naso</label><input class="input" id="cq" type="search" value="' + h(f.q) + '" data-input="catQuery"></div>' +
      '<div class="field"><label for="cfam">Famiglia</label><select class="input" id="cfam" data-change="catFam"><option value="">Tutte</option>' + E.FAMS.map(function (x) { return '<option' + (f.fam === x ? ' selected' : '') + '>' + x + '</option>'; }).join('') + '</select></div>' +
      '<div class="field"><label for="cbrand">Maison</label><select class="input" id="cbrand" data-change="catBrand" style="max-width:200px"><option value="">Tutte</option>' + brands.map(function (b) { return '<option' + (f.brand === b ? ' selected' : '') + '>' + h(b) + '</option>'; }).join('') + '</select></div>' +
      '<div class="field"><label for="cnote">Nota</label><input class="input" id="cnote" type="search" value="' + h(f.note) + '" data-input="catNote" placeholder="es. oud, vetiver" style="max-width:160px"></div>' +
      '<div class="field"><label for="cmol">Molecola</label><select class="input" id="cmol" data-change="catMol" style="max-width:180px"><option value="">Tutte</option>' + mols.map(function (m) { return '<option value="' + h(m.id) + '"' + (f.mol === m.id ? ' selected' : '') + '>' + h(m.name) + '</option>'; }).join('') + '</select></div>' +
      '<div class="field" style="max-width:100px"><label for="cyfrom">Dal</label><input class="input" id="cyfrom" type="number" placeholder="1900" value="' + h(f.yFrom) + '" data-input="catYFrom"></div><div class="field" style="max-width:100px"><label for="cyto">Al</label><input class="input" id="cyto" type="number" placeholder="2026" value="' + h(f.yTo) + '" data-input="catYTo"></div>' +
      '<div class="field"><label for="csort">Ordina per</label><select class="input" id="csort" data-change="catSort"><option value="fit"' + (f.sort === 'fit' ? ' selected' : '') + '>Affinità con me</option><option value="year"' + (f.sort === 'year' ? ' selected' : '') + '>Anno, dal più recente</option><option value="name"' + (f.sort === 'name' ? ' selected' : '') + '>Nome, A-Z</option><option value="brand"' + (f.sort === 'brand' ? ' selected' : '') + '>Maison, A-Z</option><option value="lon"' + (f.sort === 'lon' ? ' selected' : '') + '>Persistenza, dalla più lunga</option></select></div><label class="check"><input type="checkbox" data-change="catLon"' + (f.lon ? ' checked' : '') + '> Solo sopra la mia soglia di persistenza</label>' +
      (activeFilters ? '<button class="btn sm ghost" type="button" data-act="catReset">Azzera filtri</button>' : '') + '</div>' +
      '<div class="row" style="gap:4px;flex-wrap:wrap;margin-bottom:16px">' + '<button class="chip' + (!f.letter ? ' on' : '') + '" type="button" data-act="catLetter" data-letter="">Tutte</button>' + LETTERS.map(function (l) { return '<button class="chip' + (f.letter === l ? ' on' : '') + '" type="button" data-act="catLetter" data-letter="' + l + '" style="min-height:32px;padding:0 10px">' + l + '</button>'; }).join('') + '</div>' +
      '<p class="cap muted" style="margin:0 0 4px">La percentuale a destra è l’<strong>affinità</strong> con il tuo profilo attuale (note e famiglie che ami di più), non la persistenza — quella sono i pallini a sinistra (passa il mouse per il numero esatto).' + (f.mol ? ' Solo i profumi con attribuzione verificata a mano hanno una molecola collegata: gli altri non compaiono qui anche se probabilmente la contengono.' : '') + '</p>' +
      '<p class="cap muted" style="margin:0 0 14px">' + list.length + ' risultati · pagina ' + page + ' di ' + pages + '</p>' +
      '<div class="list" id="catlist">' + V.catRows(shown) + '</div>' +
      (pages > 1 ? '<div class="row" style="justify-content:center;gap:8px;padding:24px 0;flex-wrap:wrap">' +
        '<button class="btn sm" type="button" data-act="catPage" data-page="' + (page - 1) + '"' + (page <= 1 ? ' disabled' : '') + '>‹ Prec.</button>' +
        Array.from({ length: pages }, function (_, i) { return i + 1; }).filter(function (n) { return n === 1 || n === pages || Math.abs(n - page) <= 2; }).reduce(function (acc, n, i, arr) { if (i && n - arr[i - 1] > 1) acc.push('…'); acc.push(n); return acc; }, []).map(function (n) { return n === '…' ? '<span class="cap" style="align-self:center">…</span>' : '<button class="btn sm' + (n === page ? ' dark' : ' ghost') + '" type="button" data-act="catPage" data-page="' + n + '">' + n + '</button>'; }).join('') +
        '<button class="btn sm" type="button" data-act="catPage" data-page="' + (page + 1) + '"' + (page >= pages ? ' disabled' : '') + '>Succ. ›</button></div>' : '');
  };
  V.catRows = function (list) { return list.map(function (p) { var ft = E.fit(p); return C.prow(p, '<span class="row" style="gap:16px;flex-wrap:nowrap"><span class="cap" style="min-width:70px;text-align:right">' + h(p.fam) + '</span>' + C.lon(p.lon) + '<span class="pct" style="min-width:64px;text-align:right" title="Affinità con il tuo profilo">' + (ft ? Math.round(ft * 100) + '%' : '') + '</span></span>', p.nose ? 'naso: ' + h(p.nose) : ''); }).join('') || '<p class="muted" style="padding:20px 0">Nessuna scheda con questi filtri.</p>'; };
  ...
  V.profumo = function (id) {
    ...
    var out = '<section class="cols" style="margin-bottom:40px"><div class="card" style="width:min(320px,100%);flex-shrink:0;align-items:center;justify-content:center;min-height:320px;gap:18px"><span style="transform:scale(2.2);margin:50px 0">' + C.bottle(p, w && w.level != null ? w.level : 60, w && w.status === 'future') + '</span><span class="cap mono">' + h(p.conc || 'concentrazione non indicata') + '</span></div>' +
      ...
      (E.lonOk(p) ? '' : '<div class="error">Sotto la tua soglia di persistenza: il motore non te lo proporrà, ma la scheda resta consultabile.</div>') + (p.ai ? '<div class="error">Scheda compilata da Claude e non ancora verificata: controlla note e anno.</div>' : '') + (p.bulk ? '<div class="error">Scheda importata in blocco da un database pubblico: piramide reale, ma persistenza e famiglia sono stimate in automatico e possono sbagliare.</div>' : '') +
  ...
  V.maison = function (id) {
    var M = OLF.DATA.maisons;
    if (!id) {
      var q = (UI.maisonQuery || '').toLowerCase();
      var list = q ? M.filter(function (m) { return m.name.toLowerCase().indexOf(q) >= 0; }) : M;
      var top = M.slice().sort(function (a, b) { return (b.count || 0) - (a.count || 0); }).slice(0, 12);
      var byLetter = {}; list.forEach(function (m) { var l = m.name[0].toUpperCase(); (byLetter[l] = byLetter[l] || []).push(m); });
      var letters = Object.keys(byLetter).sort();
      return C.head('Maison · ' + M.length + ' case', 'Chi c’è dietro il <em>flacone</em>', 'Le 4 case raccontate per esteso vengono dai profumi curati a mano; le altre sono generate dal catalogo — nome, paese quando noto, quanti profumi hai a disposizione. Cercale o sfoglia per lettera.') +
        '<div class="field" style="max-width:420px;margin-bottom:26px"><label for="mq">Cerca una maison</label><input class="input" id="mq" type="search" value="' + h(UI.maisonQuery || '') + '" data-input="maisonQuery" placeholder="es. Tom Ford, Xerjoff, Amouage"></div>' +
        (!q ? '<section class="stack" style="margin-bottom:32px"><h2 class="h3 rule">Le più presenti nel tuo catalogo</h2><div class="row" style="gap:8px;flex-wrap:wrap">' + top.map(function (m) { return '<a class="chip" href="#/maison/' + m.id + '">' + h(m.name) + ' <span class="mono" style="font-size:11px;padding-left:6px;opacity:.7">' + (m.count || 0) + '</span></a>'; }).join('') + '</div></section>' : '') +
        '<div class="stack-lg">' + letters.map(function (l) { return '<section class="stack" style="gap:10px"><h2 class="h3 rule">' + l + '</h2><div class="row" style="gap:6px;flex-wrap:wrap">' + byLetter[l].sort(function (a, b) { return a.name.localeCompare(b.name); }).map(function (m) { return '<a class="chip' + (m.auto ? '' : ' on') + '" href="#/maison/' + m.id + '">' + h(m.name) + '</a>'; }).join('') + '</section>'; }).join('') + '</div>' +
        (!letters.length ? '<p class="muted">Nessuna maison trovata.</p>' : '') +
        '<div class="card" style="margin-top:32px"><span class="label">Case essenziere</span><p class="cap">Dove lavorano quasi tutti i nasi, e dove nascono le molecole:</p><p class="muted" style="font-size:14.5px">' + OLF.DATA.essenziere.map(h).join(' · ') + '</p></div>';
    }
    var cur = M.filter(function (m) { return m.id === id; })[0] || M[0];
    ...
    if (cur.auto) {
      return '<div class="eyebrow" style="margin-bottom:18px"><a href="#/maison">Maison</a> / ' + h(cur.name) + '</div>' +
        '<div class="cols"><div class="grow stack-lg"><header class="stack"><div class="eyebrow">Maison' + (cur.country ? ' · ' + h(cur.country) : '') + '</div><h1 class="h1" style="font-size:clamp(48px,9vw,104px)">' + h(cur.name) + '</h1>' +
        '<p class="lede">Scheda generata dal catalogo: non abbiamo ancora scritto la storia di questa maison a mano. Sappiamo cosa hai a disposizione qui dentro; il resto — fondatore, anno, proprietà — resta da cercare o da correggere.</p></header>' +
        '<div class="facts"><div><span class="k">Paese</span><span class="v">' + h(cur.country || 'non indicato') + '</span></div><div><span class="k">Nel catalogo</span><span class="v">' + perf.length + ' profumi</span></div></div>' +
        '<section class="stack-sm"><h2 class="h3 rule">Nel catalogo</h2><div class="list">' + perf.slice(0, 60).map(function (p) { return C.prow(p, '<span class="row" style="gap:14px;flex-wrap:nowrap">' + C.lon(p.lon) + '<span class="mono cap">' + (p.year || '') + '</span></span>'); }).join('') + '</div>' + (perf.length > 60 ? '<p class="cap muted">E altri ' + (perf.length - 60) + ', cercabili dal <a class="link acc" href="#/catalogo">catalogo</a> filtrando per questa maison.</p>' : '') + '</section></div></div>';
    }
    return '<div class="cols"><aside class="side-sm stack-lg"><div class="stack-sm"><div class="eyebrow" style="padding-bottom:8px"><a href="#/maison">← Tutte le maison</a></div>' + M.filter(function (m) { return !m.auto; }).map(function (m) { return '<a class="btn' + (m.id === cur.id ? ' dark' : ' ghost') + '" style="justify-content:flex-start;padding:0 12px" href="#/maison/' + m.id + '">' + h(m.name) + '</a>'; }).join('') + '</div>' +
  ...
  V.profumerie = function () {
    ...
    var svg = '<svg viewBox="0 0 860 620" role="img" aria-label="Mappa schematica del Nord Italia con le città mappate" style="display:block;width:100%;height:auto;background:var(--surface);border:1px solid var(--line);border-radius:14px">' +
  ...
  V.quaderni = function (id) {
    var A = OLF.DATA.articles, WR = OLF.DATA.weekRanges, a = A.filter(function (x) { return x.id === id; })[0];
    if (a) {
      var lk = { nota: '#/note/', profumo: '#/profumo/', molecola: '#/molecole/', maison: '#/maison/' };
      return '<article class="stack-lg"><header class="stack" style="max-width:860px"><div class="eyebrow"><a href="#/quaderni">Quaderni</a> / ' + h(a.kind) + (a.week ? ' · settimana ' + a.week + ' · ' + h(WR[a.week] || '') : '') + '</div><h1 class="h1">' + h(a.title) + '</h1><p class="lede">' + h(a.lede) + '</p>' + (a.why ? '<p class="cap muted" style="max-width:640px">Perché questa settimana: ' + h(a.why) + '</p>' : '') + '</header><div class="prose">' + a.body.map(function (p) { return '<p>' + h(p) + '</p>'; }).join('') + '</div>' +
        (a.links.length ? '<div class="stack-sm"><span class="label">Collegato nell’atlante</span><div class="row" style="gap:6px">' + a.links.map(function (l) { var label = l[0] === 'profumo' ? (E.byId(l[1]) || {}).name : (l[0] === 'molecola' ? (OLF.DATA.molecules.filter(function (m) { return m.id === l[1]; })[0] || {}).name : (l[0] === 'maison' ? (OLF.DATA.maisons.filter(function (m) { return m.id === l[1]; })[0] || {}).name : OLF.cap(l[1]))); return '<a class="chip" href="' + lk[l[0]] + encodeURIComponent(l[1]) + '">' + h(label || l[1]) + '</a>'; }).join('') + '</div></div>' : '') + '</article>';
    }
    var byWeek = {}; A.forEach(function (x) { if (x.week) byWeek[x.week] = x; });
    var now = new Date(), curWeek = Math.min(52, Math.floor((now - new Date(now.getFullYear(), 0, 1)) / 86400000 / 7) + 1);
    var curArt = byWeek[curWeek];
    return C.head('Quaderni · ' + A.length + ' su 52 settimane', 'Un quaderno a settimana', 'Niente comunicati stampa riscritti: pezzi scritti per questo atlante, uno per ogni settimana dell’anno, ciascuno con un motivo per cui è quella la settimana giusta per leggerlo. Ne mancano ancora molti: si scrivono nel tempo.') +
      (curArt ? '<a class="card night" href="#/quaderni/' + curArt.id + '" style="margin-bottom:32px"><span class="eyebrow">Questa settimana · ' + h(WR[curWeek] || '') + '</span><span class="h2">' + h(curArt.title) + '</span><span class="muted">' + h(curArt.lede) + '</span><span class="link" style="margin-top:12px">Leggi</span></a>'
        : '<div class="empty" style="margin-bottom:32px">Per la settimana ' + curWeek + ' (' + h(WR[curWeek] || '') + ') il quaderno non è ancora stato scritto.</div>') +
      '<section class="stack" style="margin-bottom:36px"><h2 class="h3 rule">Tutti i quaderni scritti finora</h2><div class="grid g2">' + A.slice().sort(function (p, q) { return (p.week || 99) - (q.week || 99); }).map(function (x) { return '<a class="card' + (x.week === curWeek ? ' night' : '') + '" href="#/quaderni/' + x.id + '" style="min-height:200px"><span class="eyebrow">' + (x.week ? 'Settimana ' + x.week + ' · ' : '') + h(x.kind) + '</span><span class="h2">' + h(x.title) + '</span><span class="muted">' + h(x.lede) + '</span><span class="link" style="margin-top:auto">Leggi</span></a>'; }).join('') + '</div></section>' +
      '<section class="stack"><h2 class="h3 rule">Il calendario delle 52 settimane</h2><p class="cap muted" style="margin-bottom:16px">Le caselle piene sono già scritte; quelle vuote arriveranno.</p><div class="weekgrid">' +
      Array.from({ length: 52 }, function (_, i) { return i + 1; }).map(function (w) { var x = byWeek[w]; return x ? '<a class="wk on' + (w === curWeek ? ' now' : '') + '" href="#/quaderni/' + x.id + '" title="' + h(x.title) + '">' + w + '</a>' : '<span class="wk' + (w === curWeek ? ' now' : '') + '" title="Settimana ' + w + ', ' + h(WR[w] || '') + ': non ancora scritto">' + w + '</span>'; }).join('') + '</div></section>' +
      '<section class="band night" style="margin-top:40px"><div class="row between" style="margin-bottom:24px;align-items:baseline"><h2 class="h2">Pellegrinaggi</h2><span class="cap">Luoghi dove un appassionato dovrebbe andare almeno una volta</span></div><div class="grid g4" style="gap:28px;grid-template-columns:repeat(auto-fit,minmax(180px,1fr))">' + OLF.DATA.pilgrimages.map(function (p) { return '<div class="stack-sm" style="border-top:1px solid var(--night-line);padding-top:16px"><div class="eyebrow">' + h(p[0]) + '</div><div class="serif" style="font-size:20px;line-height:1.2">' + h(p[1]) + '</div></div>'; }).join('') + '</div></section>';
  };
})();
